...
**بسته‌یاب** یک برنامه دسکتاپ متن‌باز، محلی‌اول و فارسی‌محور برای کشف، فیلتر، مقایسه و پیشنهاد بسته‌های اینترنت موبایل اپراتورهای ایران است.
...
بسته‌یاب بر چند اصل غیرقابل مذاکره ساخته شده است:

1. **محلی‌اول بودن:** دریافت، عادی‌سازی، کش، فیلتر، مقایسه و پیشنهاد روی دستگاه کاربر انجام می‌شود.
2. **بدون بک‌اند اختصاصی:** برنامه به سرور بسته‌یاب، حساب کاربری، پایگاه داده میزبانی‌شده یا تنظیمات راه دور نیاز ندارد.
3. **منابع رسمی مستقیم:** داده بسته‌ها باید از منابع متعلق به خود اپراتورها جمع‌آوری شود.
4. **ورودی خارجی نامطمئن است:** پاسخ‌های HTML/JSON اپراتورها اعتبارسنجی می‌شوند و تغییر ساختار آن‌ها نباید باعث کرش برنامه شود.
5. **پیشنهاد قابل توضیح:** برنامه نباید فقط یک «امتیاز مبهم» نمایش دهد؛ کاربر باید بداند پیشنهاد بر اساس قیمت، حجم عمومی، قیمت هر گیگابایت، اعتبار یا محدودیت‌ها انتخاب شده است.
6. **تفکیک حجم‌ها:** حجم عمومی، شبانه، داخلی، هدیه، اپلیکیشن‌محور یا محدود زمانی نباید بی‌صدا با هم جمع شوند و به عنوان حجم عمومی نمایش داده شوند.
...
| `cargo test --manifest-path src-tauri/Cargo.toml` | اجرای آزمون‌های Rust |
...
برای ساخت نسخه قابل توزیع:

```bash
npm run tauri build
```

Tauri ابتدا خروجی فرانت‌اند را با دستور تعریف‌شده در پیکربندی پروژه می‌سازد و سپس باینری/بسته دسکتاپ را تولید می‌کند. خروجی نهایی معمولاً در مسیرهای زیر قرار می‌گیرد:
...
نسخه منتشرشده باید به‌گونه‌ای بسته‌بندی شود که کاربر عادی بدون نصب ابزار توسعه آن را اجرا کند.
...
آزمون‌های عادی پروژه نباید به زنده بودن وب‌سایت اپراتورها وابسته باشند. منطق‌های مهم باید با داده‌های ساختگی یا fixtureهای پاک‌سازی‌شده تست شوند.
...
4. برای تغییر parser یا collector، fixture یا تست رگرسیون اضافه کنید.
5. وابستگی جدید فقط وقتی اضافه شود که واقعاً لازم باشد.
6. قالب‌بندی، typecheck و تست‌های مرتبط را اجرا کنید.
7. هیچ راز، کوکی، توکن، session id یا داده شخصی را commit نکنید.
